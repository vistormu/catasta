...
class TransformerRegressor(Module):
    def __init__(self, *,
        ...
        self.encoder_embedding = Linear(in_features=input_dim,
        ...
        self.linear_head = Linear(in_features=d_model,
    ...
    def forward(self, source: Tensor, target: Tensor | None = None) -> Tensor:
        ...
        output = self.linear_head(output.mean(dim=0))
    ...
    def decode(self, target: Tensor, memory: Tensor) -> Tensor:
        target = self.decoder_embedding(target)
        target_positional_encoding = torch.arange(target.shape[1], dtype=torch.float32, device=target.device).unsqueeze(0)
        target += target_positional_encoding
